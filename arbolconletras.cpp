...
using namespace std;
int auxX=0;
char arreglo[8]= {   'b','a','c','g','e','f','g','h'};
struct Nodo
{
    char dato;
    Nodo *der;
...
void mostrarArbol(Nodo *, char,int);
void mostrarArbol2(Nodo *, char);
...
void menu()
{
    ...
    do
    {
        ...
        cout<<"\n1 Insertar los datos del arreglo que son: arreglo[]=  {   'b','a','c','d','e','f','g','h'};\n2 Mostrar arbol de dos formas\n3 Insertar mas datos\n4 salir"<<endl;
        ...
        {
        case 1:
            cout<<"\nLETRAS DEL ARREGLO INSERTANDOS CORECTAMENTE:\n ";
            for(int i=0; i<8; i++)
            {
                dato= arreglo[i];
...
void mostrarArbol(Nodo *arbol, char cont, int posicionactual)
{
    if(arbol==NULL)
    {
        return;
...
void mostrarArbol2(Nodo *arbol, char auxY)
{
    if (arbol==NULL)
    {
        return;
